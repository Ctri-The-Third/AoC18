...
namespace AoC18.Day2Code
{
    public class Day2
    {
        ...
        public static string matchingcharacters = "";
        ...
        public static void parseProblem(string problem)
        {
            var strings = splitProblem(problem);
            ...
            foreach (var word in strings)
            {
                foreach (var word2 in strings)
                {
                    differentLetters(word, word2);
                }
            }
        ...
        public static void differentLetters (string firstBox, string secondBox)
        {
            var matchingChars = "";
            var matchingCharsCount = 0;
            for (int i = 0; i < firstBox.Length; i++)
            {
                var l1 = firstBox.Substring(i, 1);
                var l2 = secondBox.Substring(i, 1);

                if (l1.Equals(l2))
                {
                    matchingChars = matchingChars + l1;
                    matchingCharsCount++;
                }
            }

            if (matchingCharsCount == firstBox.Length - 1) //by ensuring 1 differing character, we eliminate self comparrisons.
            {
        ...
        public static string[] splitProblem (string problem)
        {
        ...
        public static void checkstring(string sentence)
        {
            ...
            if (twos /2 > 0) { Day2.twos++; }
            if (threes /2 > 0) { Day2.threes++; }
        }
